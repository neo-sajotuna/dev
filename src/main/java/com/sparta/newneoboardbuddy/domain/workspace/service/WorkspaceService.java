...
public class WorkspaceService {
    ...
    public WorkspaceResponse createWorkspace(AuthUser authUser, WorkspaceRequest workspaceRequest) {
        ...
        if (user.getUserRole() != UserRole.ROLE_ADMIN) {
            throw new IllegalArgumentException("워크스페이스를 생성할 권한이 없습니다.");
        }
    ...
    public void inviteMember(AuthUser authUser, Long spaceId, String email, MemberRole memberRole) {
        ...
        Member member = memberRepository.findByUserIdAndWorkspaceId(user.getId(), spaceId)
                .orElseThrow(() -> new IllegalArgumentException("해당 워크스페이스에 대한 권한이 없습니다."));

        // 멤버 초대는 워크스페이스 역할을 가진 멤버만 가능
        if (member.getMemberRole() != MemberRole.WORKSPACE_MEMBER) {
            throw new IllegalArgumentException("워크스페이스에 멤버 초대할 권한이 없습니다.");
        }
        ...
        throw new IllegalArgumentException("해당 유저는 이미 이 워크스페이스의 멤버입니다.");
        }
    ...
    public UpdateWorkspaceResponse updateWorkspace(AuthUser authUser, Long spaceId, WorkspaceRequest workspaceRequest) {
        ...
        if (member.getMemberRole() != MemberRole.WORKSPACE_MEMBER) {
            throw new IllegalArgumentException("워크스페이스를 수정할 권한이 없습니다.");
        }

        //워크스페이스가 존재하는지
        Workspace workspace = workspaceRepository.findById(spaceId)
                .orElseThrow(() -> new IllegalArgumentException("해당 ID의 워크스페이스를 찾을 수 없습니다."));
    ...
    public void deleteWorkspace(AuthUser authUser, Long spaceId) {
        ...
        // 유저가 워크스페이스를 수정할 권한이 없는 경우 (예: WORKSPACE_MEMBER 아닌 경우)
        if (member.getMemberRole() != MemberRole.WORKSPACE_MEMBER) {
            throw new IllegalArgumentException("워크스페이스를 삭제 권한이 없습니다.");
        }
